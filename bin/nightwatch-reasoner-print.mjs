...
const prompt = [
  ...
  'On OBSERVE/PLAN, CALL_TOOL an allowed tool. Empty INSPECT_SOURCE_SURFACE arguments return the file index; then inspect one listed path. After source results exist, FORM_HYPOTHESIS using observed refs.',
  'On VERIFY, call RERUN_SAFE_REPRODUCTION with the inspected path/ref, then REQUEST_FINDING_PROPOSAL, then PROPOSE_CANDIDATE with observed evidence refs. Do not invent evidence refs.',
