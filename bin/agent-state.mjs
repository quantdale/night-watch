...
function loadGitGraph(root) {
  if (cachedGitGraph !== undefined) return cachedGitGraph;
  try {
...
// within a single read-only checker process, so the first answer is cached.
let cachedOriginMainHead;
function liveOriginMainHead(root) {
  if (cachedOriginMainHead === undefined) {
    cachedOriginMainHead = commandOutput(root, ['rev-parse', 'origin/main'])?.trim() ?? null;
  }
  return cachedOriginMainHead;
}
