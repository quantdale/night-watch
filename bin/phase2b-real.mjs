...
try {
  ...
  if (capability.budgetWarning) {
    console.error(`[phase2b] WARNING ${capability.budgetWarning.code}: remaining validity ${capability.budgetWarning.remainingValidityMs} ms is shorter than the declared run budget ${capability.budgetWarning.requiredValidityMs} ms`);
  }
} catch (error) {
  if (error && typeof error === 'object' && typeof error.code === 'string' && error.code.startsWith('AUTH_CAPABILITY_')) {
    console.error(`[phase2b] REFUSED ${error.code}: ${error.message}`);
    process.exit(3);
  }
  throw error;
}
