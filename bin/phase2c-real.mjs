...
if (env === undefined || storage === undefined || storage.trim() === '') {
  throw new Error('phase2c-real requires exactly one --env=dev|next and --storage-state=/absolute/external/state.json');
}
...
const run = spawnSync(cmd, ['test', '--config=playwright.phase2c.config.ts', '--project=nightwatch', '--workers=1'], { cwd: root, env: commonEnv, stdio: 'inherit' });
