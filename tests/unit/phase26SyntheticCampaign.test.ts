...
test('Phase 26 source proof reaches the existing Phase24 semantic/replay/dossier path offline', () => {
  ...
  try {
    ...
    const replayPlan = createPhase24ReplayPlan({ candidateId: candidate.candidateId, candidateDecisionDigest: candidate.deterministicDigest, occurrenceIdentity: 'occurrence.phase26.first', source: candidate.source, semanticContractId: candidate.contract.contractId, expectationId: expectation.expectationId, sanitizedObservationDigest: 'observation:sha256:' + '2'.repeat(24), executionPrerequisites: ['SOURCE_CURRENT', 'READ_ONLY_PROVEN'] });
