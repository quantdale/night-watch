// ---------------------------------------------------------------------------
// Phase 10B contained DEV deep-semantic acceptance runner (owner-authorized:
...
// verified DEV URL, evaluating the CURRENT real-source v2 DEEP expectation
// ripple.common-exchange.read.real-source-deep through the Phase 9A.1
// semantic channel. No journey selector, no expectation selector, no target
// selector, no fallback, no third attempt.
//
// The L3 item type contract itself must be DECISIVE: for a clean PASS every
// current invariant of the resolved deep expectation must evaluate PASS with
// zero N/A (a root-array PASS alone is NOT deep validation; an empty
// top-level array is NOT acceptance — PHASE_10B_BLOCKED_DEEP_INVARIANT_NOT_
// OBSERVED). The expected invariant total is DERIVED from the resolved
// expectation (never hard-coded independently of the registry).
//
// Pre-browser hard gates (metadata-only, no product contact):
//   - one-shot gate NIGHTWATCH_PHASE_10B_REAL=1 (the launcher sets it)
//   - read-only remote source freshness (gh api; disposable /tmp mirror)
//   - mechanical derivation of the DEEP expectation at the freshness-approved
//     snapshot (incl. the deep TYPE_MATCH OBJECT item contract)
...
//
// The runner reuses the established Phase 2B/9B machinery: runRealRunGate,
// createNightwatchContext, runDeclarativeJourney, endpoint registry, auth
// readability checks, replay comparison, proxy/safety accounting, and the
// pure Phase 9B freshness/preflight/summary mechanics. It does NOT become a
// parallel browser framework, and it never broadens runtime authority beyond
// the already-validated Phase 9B containment.
// ---------------------------------------------------------------------------
...
import { classifySourceFreshness, type Phase9bFreshnessVerdict } from '../../src/core/phase9b/freshness';
...
// ---------------------------------------------------------------------------
// Phase 10B fixed acceptance identity (authorization §6; no selectors).
// ---------------------------------------------------------------------------
const SELECTED_JOURNEY_ID = 'ripple-common-exchange-read';
const SELECTED_TARGET_ID = 'ripple.common-exchange.read';
const SELECTED_EXPECTATION_ID = 'ripple.common-exchange.read.real-source-deep';
...
const DISPOSABLE_ROOT = process.env.NIGHTWATCH_PHASE_10B_DISPOSABLE_ROOT ?? '/tmp/nightwatch-phase10b-source';
const DISPOSABLE_MARKER = '.nightwatch-phase10b-sha';

/** Relevant common-exchange contract paths (Phase 9B §7, reused verbatim for
 *  the same journey). */
const API_RELEVANT_PATHS = ['src/App/Handler/ExchangeRate.php', 'src/App/Route/Config/Routing.yaml'];
/** Journey callsite / route / page paths (Phase 9B §7, reused verbatim for
 *  the same journey). */
...
// ---------------------------------------------------------------------------
// Local helpers (read-only; no product contact). Mirrors the Phase 9B
// runner's validated orchestration with the Phase 10B fixed identity.
// ---------------------------------------------------------------------------
...
function canonicalTarget(env: EnvironmentConfig): string {
  ...
  if (override !== undefined && override.trim() !== '') {
    throw new Error('fail-closed: Phase 10B uses the canonical fixed DEV URL only; NIGHTWATCH_UI_URL is not accepted');
  }
...
function ensureDisposableSnapshot(repoId: string, sha: string): string {
  ...
  // Synthetic pinned ref: HEAD -> refs/heads/phase10b-fresh -> <exact SHA>.
  fs.mkdirSync(path.join(dir, '.git', 'refs', 'heads'), { recursive: true });
  fs.writeFileSync(path.join(dir, '.git', 'HEAD'), 'ref: refs/heads/phase10b-fresh\n');
  fs.writeFileSync(path.join(dir, '.git', 'refs', 'heads', 'phase10b-fresh'), `${sha}\n`);
...
/** Build the Phase 10B resolver exposing ONLY the selected target, bound to
 *  the freshness-approved snapshot, and REQUIRE the resolved expectation to
 *  carry the current deep L3 item type contract. Returns the oracle, the
 *  resolution, and the derived expected invariant total (from the resolved
 *  expectation — never hard-coded independently). */
function buildPhase10bOracle(approvedSha: string): {
  oracle: SemanticHookOracle;
  resolution: RealSourceResolution;
  expectation: SemanticExpectation;
  expectedInvariantTotal: number;
} {
  const access = createSiblingSourceAccess(DISPOSABLE_ROOT);
  ...
  if (selected.length !== 1) {
    throw new Error(`PHASE_10B_BLOCKED_EXPECTATION_NOT_RESOLVED: selected target derived ${selected.length} expectations (require exactly 1)`);
  }
  const expectation = selected[0]!;
  if (expectation.expectationId !== SELECTED_EXPECTATION_ID) {
    throw new Error(`PHASE_10B_BLOCKED_EXPECTATION_NOT_RESOLVED: derived expectation id ${expectation.expectationId} != ${SELECTED_EXPECTATION_ID}`);
  }
  if (expectation.sourceProvenance.evidenceDigest === undefined) {
    throw new Error('PHASE_10B_BLOCKED_EXPECTATION_NOT_RESOLVED: source evidence digest missing (not an admitted real-source expectation)');
  }
  if (expectation.sourceProvenance.sha !== approvedSha) {
    throw new Error(`PHASE_10B_BLOCKED_EXPECTATION_NOT_RESOLVED: expectation bound to ${expectation.sourceProvenance.sha} != approved ${approvedSha}`);
  }
  ...
  if (resolution.kind !== 'RESOLVED') {
    throw new Error(`PHASE_10B_BLOCKED_EXPECTATION_NOT_RESOLVED: resolver returned ${resolution.kind}`);
  }
  return { oracle: { resolve: (input) => resolver.resolve(input) }, resolution, expectation, expectedInvariantTotal };
}
...
function exactImplementationCiGreen(headSha: string): { green: boolean; detail: string } {
  const runId = process.env.NIGHTWATCH_PHASE_10B_CI_RUN_ID;
  if (runId === undefined || runId.trim() === '') {
    return { green: false, detail: 'NIGHTWATCH_PHASE_10B_CI_RUN_ID not provided' };
  }
  try {
...
async function observeOnce(opts: {
  ...
  if (!auth.valid) {
    throw new Error(`PHASE_10B_BLOCKED_HUMAN_AUTH_ACTION_REQUIRED: ${opts.pass} auth gate; run ${AUTH_CAPTURE_COMMAND}`);
  }
  ...
  const recorder = new RunRecorder({
    ...
    product: 'ripple',
    browser: 'chromium',
    scenario: `ripple-phase-10b-${opts.definition.journeyId}-${opts.pass}`,
    nightwatchSha: nightwatchSha(root),
    authenticated: true,
  });
  recorder.addManifestEntry('phase10bJourney', {
  ...
  recorder.event({
    type: 'start',
    severity: 'info',
    message: 'Phase 10B controlled deep-semantic journey started',
    data: {
      journeyId: opts.definition.journeyId,
      targetId: SELECTED_TARGET_ID,
  ...
  recorder.finalize({
    passed,
    notes: [
      `Phase 10B ${opts.definition.journeyId} ${opts.pass} controlled deep-semantic observation`,
      ...
      `semantic summary: receipts=${summary.receiptCount} resolved=${summary.resolvedExpectationCount} PASS=${summary.passCount} ANOMALY=${summary.anomalyCount} N/A=${summary.notApplicableCount} decisive=${summary.decisiveEvaluationCount} invariants=${summary.invariantTotal}/${summary.invariantPassCount}/${summary.invariantNaCount}/${summary.invariantViolationCount}`,
    ],
  });
  ...
  for (const dir of runDirs) {
    ...
    for (const entry of entries) {
      if (/\.(png|jpe?g|webp|zip|webm)$/i.test(entry)) {
        throw new Error(`PHASE_10B_BLOCKED_PRIVACY: persisted media artifact ${entry} in ${dir}`);
      }
      if (/storage[_-]?state/i.test(entry)) {
        throw new Error(`PHASE_10B_BLOCKED_PRIVACY: storage-state copy ${entry} in ${dir}`);
      }
    }
    for (const file of ['events.jsonl', 'network.jsonl']) {
      ...
      for (const line of fs.readFileSync(eventsPath, 'utf8').split(/\r?\n/)) {
        ...
        try {
          ...
          throw new Error(`PHASE_10B_BLOCKED_PRIVACY: unparseable evidence line in ${file}`);
        }
        ...
        for (const key of Object.keys(data)) {
          if (/^(rawbody|rawtext|body|responsebody|cookies|storage_state)$/i.test(key)) {
            throw new Error(`PHASE_10B_BLOCKED_PRIVACY: forbidden evidence key "${key}" in ${file}`);
          }
        }
      }
    }
...
test('Phase 10B contained DEV deep-semantic acceptance: common-exchange deep FIRST + replay', async ({ browser }) => {
  if (process.env.NIGHTWATCH_PHASE_10B_REAL !== '1') {
    throw new Error('fail-closed: Phase 10B real acceptance requires NIGHTWATCH_PHASE_10B_REAL=1 and the gated launcher');
  }
  const envName = assertSupportedEnvironment(process.env.NIGHTWATCH_ENV);
  if (envName !== 'dev') throw new Error('fail-closed: Phase 10B real acceptance requires DEV; NEXT and production are forbidden');
  ...
  // --- Pre-browser source freshness (authorization §13-§17) ---
  const remoteApiSha = remoteBranchHead(RIPPLE_API_REPO, RIPPLE_API_REMOTE_BRANCH);
  const remoteUiSha = remoteBranchHead(RIPPLE_UI_REPO, RIPPLE_UI_REMOTE_BRANCH);
  if (remoteApiSha === null || remoteUiSha === null) {
    throw new Error('PHASE_10B_BLOCKED_SOURCE_FRESHNESS_UNRESOLVED: remote branch head unavailable; no DEV contact');
  }
  ...
  if (freshness.verdict.kind === 'BLOCK') {
    throw new Error(`${phase10bFreshnessBlockToken(freshness.verdict.reason)}: ${freshness.verdict.reason} (derivation ${JSON.stringify(freshness.derivation)})`);
  }

  // --- Build the admitted resolver for the selected target only and REQUIRE
  // RESOLVED with the deep contract immediately before browser launch ---
  const { oracle, resolution, expectation, expectedInvariantTotal } = buildPhase10bOracle(freshness.approvedSha);
  if (resolution.kind !== 'RESOLVED') {
    throw new Error(`PHASE_10B_BLOCKED_EXPECTATION_NOT_RESOLVED: pre-launch resolution ${resolution.kind}`);
  }
  if (resolution.sourceSnapshot.sha !== freshness.approvedSha) {
    throw new Error(`PHASE_10B_BLOCKED_EXPECTATION_NOT_RESOLVED: snapshot ${resolution.sourceSnapshot.sha} != approved ${freshness.approvedSha}`);
  }

  // --- Pre-dev metadata-only readiness gate (reuses the Phase 9B gate) ---
  ...
  expect(semanticComparison.pass, `PHASE_10B_BLOCKED_SEMANTIC_NONDETERMINISM: ${semanticComparison.mismatches.join(',')}`).toBe(true);
  ...
  // --- Privacy audit + safe acceptance evidence (authorization §39, §40) ---
  assertNoSensitiveArtifacts([path.join(root, 'artifacts', first.runId), path.join(root, 'artifacts', replay.runId)]);
  const acceptanceEvidence = {
    schemaVersion: 'nightwatch.phase10b-acceptance.v1',
    ...
    deepContract: {
      ...
      kind: PHASE_10B_DEEP_TYPE_CONTRACT.kind,
      expectedType: PHASE_10B_DEEP_TYPE_CONTRACT.expectedType,
    },
    freshness: {
    ...
    ciRunId: process.env.NIGHTWATCH_PHASE_10B_CI_RUN_ID ?? null,
    first: {
      runId: first.runId,
      ...
      invariantTotal: first.summary.invariantTotal,
      invariantPassCount: first.summary.invariantPassCount,
      invariantNaCount: first.summary.invariantNaCount,
      invariantViolationCount: first.summary.invariantViolationCount,
    ...
    replay: {
      ...
      invariantTotal: replay.summary.invariantTotal,
      invariantPassCount: replay.summary.invariantPassCount,
      invariantNaCount: replay.summary.invariantNaCount,
      invariantViolationCount: replay.summary.invariantViolationCount,
      findingCount: replay.summary.findingCount,
    ...
    deepInvariantDevValidation: 'VERIFIED',
    productSemanticMismatch: 'NONE_OBSERVED',
  };
  const artifactsDir = path.join(root, 'artifacts');
  fs.mkdirSync(artifactsDir, { recursive: true });
  fs.writeFileSync(path.join(artifactsDir, `${baseRunId}-phase10b-acceptance.json`), JSON.stringify(acceptanceEvidence, null, 2));
});
