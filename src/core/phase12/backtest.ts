...
import type { MinimizationAction, MinimizationResult, AnomalyObservation } from '../triage/types';
...
function syntheticReplay(targetFp: string, opts: {
  ...
  return async (seq: readonly MinimizationAction[], phase: 'FRESH_EXACT_REPLAY' | 'REDUCED_CANDIDATE') => {
    ...
    if (opts.emptyInvalid && seq.length === 0) return { status: 'INVALID' as const, safety: zero, invalidReason: 'PRECONDITION_DIVERGENCE' as const };
    if (opts.preconditionDivergence && phase === 'REDUCED_CANDIDATE' && seq.length === 1) return { status: 'INVALID' as const, safety: zero, invalidReason: 'PRECONDITION_DIVERGENCE' as const };
    if (opts.safetyNonZero) return { status: 'FAILURE' as const, anomalyFingerprint: targetFp, safety: { ...zero, proxyViolations: 1 } };
    if (opts.sameFingerprint === false) return { status: 'FAILURE' as const, anomalyFingerprint: opts.differentFingerprintValue ?? fp('different'), safety: zero };
    return { status: 'FAILURE' as const, anomalyFingerprint: targetFp, safety: zero };
  };
}

function baselineReplay(): (seq: readonly MinimizationAction[], phase: 'FRESH_EXACT_REPLAY' | 'REDUCED_CANDIDATE') => Promise<ReturnType<typeof syntheticReplay> extends (...args: unknown[]) => Promise<infer R> ? R : never> {
  return async (seq, phase) => {
    const zero = { productionAttempts: 0, proxyViolations: 0, unknownDestinations: 0, unknownApprovals: 0, knownMutations: 0, actionCausedUnknown: 0, dbQueries: 0 };
    if (phase === 'FRESH_EXACT_REPLAY') {
      // fresh still reproduces (baseline flaw is reduced only)
      return { status: 'FAILURE' as const, anomalyFingerprint: fp('baseline'), safety: zero };
    }
    return { status: 'INVALID' as const, safety: zero, invalidReason: (seq.length === 0 ? 'PRECONDITION_DIVERGENCE' : 'ACTION_NOT_APPROVED') as const };
  };
}
...
export interface BacktestCase {
  ...
  readonly replayOpts: Parameters<typeof syntheticReplay>[1];
  readonly preconditionCheck?: (seq: readonly MinimizationAction[]) => { valid: boolean; reason?: string };
...
export function buildFixedCorpus(): BacktestCase[] {
  ...
  // 27 privacy sentinel (body contains sentinels, pipeline must not leak)
  cases.push({ id: 'c27-privacy-sentinel', kind: 'PRIVACY_SENTINEL', responseBody: privacyAdversarialResponse(), expectation: PHASE12_EXPECTATIONS.commonTypeMonth, sourceSha: PHASE12_FIXTURE_SHA, targetFingerprint: actionableFp, originalActions: actions(2), isActionableDefect: false, isBenign: true, isFalsePositiveFixture: false, replayOpts: {}, candidateKind: 'JOURNEY' });
  return cases;
}
...
function confidenceBlockedBySemantics(sem: { outcome: string }, knownFalsePositive: boolean, safetyClean: boolean): boolean {
  ...
  if (sem.outcome === 'PARTIAL_COVERAGE') return true;
  if (sem.outcome === 'EXPECTATION_SOURCE_STALE' || sem.outcome === 'EXPECTATION_SOURCE_UNAVAILABLE' || sem.outcome === 'EXPECTATION_UNAVAILABLE') return true;
  if (sem.outcome === 'NO_EXPECTATION') return true;
  return false;
}
...
export async function runBacktestOnce(opts: { baseline: boolean }): Promise<{ metrics: BacktestMetrics; raw: unknown[]; determinismKey: string }> {
  ...
  for (const c of corpus) {
    ...
    const replayFn = isBaseline ? baselineReplay() as unknown as typeof syntheticReplay extends (...a: infer A) => unknown ? (...a: A) => Promise<{ status: string; anomalyFingerprint?: string; safety: { productionAttempts: number; proxyViolations: number; unknownDestinations: number; unknownApprovals: number; knownMutations: number; actionCausedUnknown: number; dbQueries: number }; invalidReason?: string }> : never : syntheticReplay(target, c.replayOpts) as unknown as typeof syntheticReplay extends (...a: infer A) => unknown ? (...a: A) => Promise<{ status: string; anomalyFingerprint?: string; safety: { productionAttempts: number; proxyViolations: number; unknownDestinations: number; unknownApprovals: number; knownMutations: number; actionCausedUnknown: number; dbQueries: number }; invalidReason?: string }> : never;
    // For baseline same-fp modeling we need to pass target — baselineReplay uses fixed fp; adjust
    const effectiveReplay: typeof replayFn = isBaseline
      ? (async (seq: readonly MinimizationAction[], phase: 'FRESH_EXACT_REPLAY' | 'REDUCED_CANDIDATE') => {
          const zero = { productionAttempts: 0, proxyViolations: 0, unknownDestinations: 0, unknownApprovals: 0, knownMutations: 0, actionCausedUnknown: 0, dbQueries: 0 };
          if (phase === 'FRESH_EXACT_REPLAY') return { status: 'FAILURE' as const, anomalyFingerprint: target, safety: zero };
          return { status: 'INVALID' as const, safety: zero, invalidReason: (seq.length === 0 ? 'PRECONDITION_DIVERGENCE' : 'ACTION_NOT_APPROVED') as const };
        }) as unknown as typeof replayFn
      : replayFn;
    ...
    // dossier readiness (simplified deterministic gate)
    const ready = semOutcome === 'ANOMALY' && reproduced && safetyClean && !c.isFalsePositiveFixture && !blocked && level !== 'UNRESOLVED';
    ...
    if (!isBaseline) {
      ...
      if (c.kind === 'DIFFERENT_FP' && reproduced && c.replayOpts.sameFingerprint === false) {
        // if we ever count this as reproduced, it's a false reproduction
        // our synthetic replay already returns different fp -> minimization will mark DOES_NOT_REPRODUCE, so this branch should stay zero
        if (minimization?.freshExactReplay === 'REPRODUCED' && minimization.reproductionCount > 0) {
          // check the replay's fingerprint equality: different fp case uses same target but replay returns other — outcomeReason will mark mismatch as DOES_NOT
          // so to prove false-repro stays zero we only increment if somehow still marked repro
