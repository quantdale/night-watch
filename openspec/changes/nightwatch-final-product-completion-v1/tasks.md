...
- [x] — DONE 2026-09-26: `gate:clean` runs sibling-absent by default (empty disposable root per run; `NIGHTWATCH_CLEAN_SIBLING_MODE=OPT_IN_SIBLING` is the explicit recorded opt-in that must name its root); the receipt replaced `siblingWrites: 0` with measured before/after sibling identity digests and drift fails with SIBLING_IDENTITY_DRIFT (R2-N3); a real clean-checkout run produced the receipt with siblingMode=ABSENT, identity unchanged, install PASS. `gate:clean` runs sibling-absent by default (explicit, recorded opt-in for sibling mode). Replace the literal `siblingWrites: 0` with a measured before/after sibling identity (R2-N3).
...
- [x] — DONE 2026-09-26: `SESSION_DECLARED_ABSENT_EXPECTED` in ci/clean modes only — the classification requires the gate label NIGHTWATCH_GATE_ENVIRONMENT ∈ {CI, CLEAN}, the declared branch equal to the task STATE branch, and no prior routing error; the label is forwarded through the handoff and project-state continuity spawns, narrowly permitted in the handoff env rule (D-04), declared in the environment surface, with positive/negative unit probes (mismatched branch and campaign drift still fail hard in ci mode) and a live clone proof (CI mode PASS / local mode FAIL). HANDOFF_TRUTH `SESSION_DECLARED_ABSENT_EXPECTED` in ci/clean modes, only when the declared branch equals STATE's branch. Add a negative probe for a mismatched branch (D-04).
- [x] 4.11 Pin actions by full SHA; anchored rule over every workflow file and compact step forms; move CI to Node 22 on Node-24-native action majors with `runs-on: ubuntu-24.04`; align the `gate:clean` Node path; record the exact node/npm versions in receipts (NW-AUD-001, NW-AUD-004 narrowed, D-19).
