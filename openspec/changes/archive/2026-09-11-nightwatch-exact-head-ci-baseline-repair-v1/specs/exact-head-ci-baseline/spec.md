...
- **WHEN** a GitHub Actions run bootstraps the runner and executes repository code at the current substantive baseline
- **THEN** it SHALL be classified as executed CI, and a run that executes no steps SHALL remain a zero-step platform block rather than repository validation evidence.
