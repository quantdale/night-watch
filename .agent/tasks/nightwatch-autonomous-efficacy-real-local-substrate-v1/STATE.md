...
Documentation descendant: 36c46fc11017eb95f42821d9233da53ddc8a57a5 — documentation-only (verified: `git diff 3d624fb..36c46fc` touches nothing outside `.agent/` and `docs/`). `gate:local` re-run there is also FULL PASS 11/11, receipt `receipt:sha256:60574f3713c36d6bfd39d096`, so the certification covers the head that carries these records.
