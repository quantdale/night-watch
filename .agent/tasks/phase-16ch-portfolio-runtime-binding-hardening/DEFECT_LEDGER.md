...
- HYP-07 (complete-suite import/topology fallout): not yet finally dispositioned — pre-commit canonical complete ran 2230 passed / 2 failed / 4 skipped where BOTH failures are the selfDev dirty-tree precondition (`SELFDEV_AUTHORITATIVE_SOURCE_DIRTY` on uncommitted hardening work), not import fallout; final clean-tree canonical + isolated runs are owed below.
- HYP-08 (isolated environment altering determinism/file rejection): pending isolated run (M9).
