...
Task ID: nightwatch-semantic-gap-ledger-integrity-v1
Phase: SEMANTIC_GAP_LEDGER_INTEGRITY_V1
Status: COMPLETE
...
PHASE_SEMANTIC_GAP_LEDGER_INTEGRITY_V1_STATUS: COMPLETE

## Objective

Produce the apply-ready plan for NW-AUD-042 without implementation.
...
| Path | Reason | Status |
|---|---|---|
| `openspec/changes/nightwatch-semantic-gap-ledger-integrity-v1/` | remediation planning | complete |
| `.agent/tasks/nightwatch-semantic-gap-ledger-integrity-v1/` | planning continuity | complete |
...
Command: static graph/ledger/quality/test inspection
Result: SUBSTANTIATED READ-ONLY
Relevant failure/output summary: self-certified closure, omitted new gaps, lossy joins, inconsistent counts, and duplicate-surface inflation confirmed.

## Decisions Made During This Task

Decision: admit NW-AUD-042 at Medium severity and High confidence.
Reason: the defect corrupts local closure and prioritization truth but does not directly grant source or execution authority.

## Discoveries

- `remainingGapCount` is taken from graph length while other summaries use the incomplete rebuilt record set.
