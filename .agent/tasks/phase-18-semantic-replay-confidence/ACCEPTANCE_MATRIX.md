...
| A07 | Replay distinguishes repeated occurrences and refuses ambiguous binding | replay V3 matrix |
| A08 | Actual synthetic minimization preserves the same semantic anomaly identity | reducer/replay evidence |
| A09 | Replay outcomes distinguish product divergence, precondition divergence, infra failure, and invalid input | taxonomy matrix |
| A10 | HIGH confidence has explicit evidence gates and adversarial inflation attempts fail | confidence matrix |
| A11 | Confidence degrades when source/expectation/replay evidence becomes stale | degradation matrix |
| A12 | Semantic clustering is deterministic and separates materially different contracts | clustering matrix |
| A13 | Change impact connects to semantic coverage with bounded explanations and no new authority | portfolio integration tests |
| A14 | Coverage accounting is deterministic and non-authoritative | metrics corpus |
