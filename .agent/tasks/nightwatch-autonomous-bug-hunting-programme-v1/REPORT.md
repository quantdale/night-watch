...
- Tests/validation:
  ...
  - `INSPECT_SOURCE_SURFACE` now returns a file index with first-line previews, then a single pre-fix file when `arguments.path` matches. Print prompt shows the path example. Aqua-ui re-hunt: Grok inspected `AwsRiUtilizationCharts.vue` then `UtilizationCharts.vue`, admitted=true, outcome=MISS, leak=[], keywordRecall=0, fileHits=0, HYPOTHESIZE TIMEOUT. Retrieval is used; rediscovery is not.
