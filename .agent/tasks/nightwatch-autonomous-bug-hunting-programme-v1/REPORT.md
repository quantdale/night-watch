...
- Tests/validation:
  ...
  - `INSPECT_SOURCE_SURFACE` now returns a file index, then a single pre-fix file when `arguments.path` matches. Aqua-ui re-hunt: still MISS, leak=[], not admitted. Retrieval is real; rediscovery is not.
