...
- Tests/validation:
  ...
  - Same harness re-measure of two earlier cases, not cherry-picks: `alphauslabs/Octo-Design-System` fileHits=1/fileTotal=1 SAME_ROOT_CAUSE_ALTERNATE; `alphauslabs/ai-driven-bug-hunting` fileHits=1/fileTotal=2 SAME_ROOT_CAUSE_ALTERNATE. leak=[] all. keywordRecall=0 all. This bucket is file-mention recall against the pre-fix snapshot (the fix commit's files). Hidden rootCause is null on git-mined records, so EXACT/PARTIAL remain unreachable. Historical rediscovery of the actual defect is still NOT PROVEN.
