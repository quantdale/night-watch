...
2. `.agent/tasks/nightwatch-autonomous-efficacy-real-local-substrate-v1/{SPEC,PLAN,STATE,REPORT}.md`
3. `.agent/tasks/nightwatch-real-local-investigation-substrate-v1/{STATE,REPORT}.md`
4. parent `PROGRAMME.json`, `STATE.md`, `REPORT.md`
5. `AGENTS.md`, applicable instructions, `docs/CURRENT_STATE.md`, OpenSpec, then live Git/workspace/session truth.
...
- final dossier admission is mechanical;
...
IMPLEMENTATION AUTHORIZED:
  ...
  additive reasoning-memory/campaign-strategy/readiness/efficacy logic,
  owner-local read-only providers, historical benchmark/replay harness,
  synthetic fixtures and negative controls, commits/pushes/integration,
  local and clean-clone certification.
...
Repository-local/offline except for already-authorized subscribed CLI provider calls used as the reasoner. C-00 governs: implementation happens only in owned session worktrees; the canonical checkout is not an implementation worktree.
