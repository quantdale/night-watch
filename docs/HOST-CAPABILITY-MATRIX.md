...
| Owner-captured authenticated storage state | `src/auth/capabilityLifecycle.ts` (`nightwatch.auth-capability-record.v1` / `nightwatch.auth-capability-preflight.v1`) | every authenticated lane: the ten authenticated owner-manual harnesses, `journey:phase2c`, `explore:phase4`, `api:phase5`, `campaign:real`, Phase 9B/10B DEV semantics, C-12 passive observation. Exempt (never read the owner artefact): six `LOCAL_FIXTURE_SMOKE` checks, `tests/manual/auth-capture.synthetic.ts`, `tests/manual/phase2a-canary.ts` | `UNAVAILABLE_CAPABILITY` with acquisition condition "re-capture for this environment" (`npm run auth:capture -- --env=<env> --output=<path>`); `MISSING`/`UNREADABLE`/`UNKNOWN_AGE`/`EXPIRED`/`WRONG_ENVIRONMENT` refuse before any browser, subprocess, socket or file. The sidecar carries no cookie value, token, header or storage value, and a refusal is never a failure of the code |
