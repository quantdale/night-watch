...
> Durable memory for the next agent/session. Last updated: **2026-09-25**
...
| `W7_REAL_LOCAL_INVESTIGATION_SUBSTRATE_STATUS` | `COMPLETE_LOCAL_NOT_CI_VERIFIED` — the ordinary `nightwatch-agent campaign run` path now senses real owner-local source/System Map/mined Bug Atlas/explicit-real System Atlas/sanitized evidence through `src/core/localInvestigation/**` fail-closed providers, shares one `DeterministicReproductionProvider` with the historical benchmark, and can only emit a dossier through `admitLocalFinding` (forged `reproductionCount` and invented evidence refs are refused). Additive `VERIFIED_ROOT_CAUSE_REDISCOVERY` tier; strict `EXACT_REDISCOVERY` unchanged and still 0. Durable programme identity validated pre-parse by `bin/lib/programme-state.mjs`. Implementation checkpoint `e368f9255142d1b30dd66825d93f6f321ba6ecbf`, certification observed at `e368f9255142d1b30dd66825d93f6f321ba6ecbf`: regression 4378/0/14, `gate:local` 11/11 receipt `receipt:sha256:85ace28cea2d1e5de2af9723`, `gate:clean` PASS Node 20 inner receipt `receipt:sha256:c6f3ecdc4d45b7494d961272`. Product-path historical proof reached `dossierStatus=VERIFIED_REPRODUCTION` / `reproductionCount=1` with zero leakage and zero sibling mutation; live-provider efficacy remains unproven (`NO_PROGRESS`, 0 candidates). REPORT in `.agent/tasks/nightwatch-real-local-investigation-substrate-v1/` |
| `W8_AUTONOMOUS_EFFICACY_REAL_LOCAL_SUBSTRATE_STATUS` | `COMPLETE_LOCAL_NOT_CI_VERIFIED` — autonomous investigation efficacy on the W7 substrate. Stateless CLI turns now receive bounded per-turn `nightwatch.investigation-memory.v1` (inspected targets WITH their grounding evidence refs, exhausted targets, salient symbols, hypothesis progress, reproduction readiness, stagnation risk, deterministic advisory directives) through `reasoner-turn-request.v2`, and fresh investigations inherit bounded `nightwatch.campaign-strategy-state.v1` with fail-closed resume. Memory is derived per turn, never persisted, and grants no authority: only a SUCCESSFUL source-surface result promotes an approved target, and `SUPPORTED`/`DISPROVED` require an observed reproduction verdict. Fixed corpus `nightwatch.efficacy-corpus.v2` (13 cases, 2 negative controls, one commit, one investigator policy, differing only by the mechanical `projectRequestToW7` projection) moved baseline -> final: unique source targets 13 -> 31, grounded hypotheses 0 -> 17, verification-ready 0 -> 17, grounded reproduction attempts 0 -> 17, mechanical reproductions 0 -> 6, admitted candidates 0 -> 6, disproved hypotheses 0 -> 6, `VERIFIED_ROOT_CAUSE_REDISCOVERY` 0 -> 3, false positives 0 -> 0, leaked cases 0 -> 0, `EXACT_REDISCOVERY` 0 -> 0 (unchanged, nothing leaked to raise it); costs rose honestly (reasoner calls 39 -> 102, tool actions 26 -> 67, stagnation terminations 0 -> 1). Live `opencode-go/deepseek-v4-flash`: the leak-isolated historical proof reached `VERIFIED_ROOT_CAUSE_REDISCOVERY` with mechanical reproduction on 2 of 3 cases with the negative control candidate-free, and two real owner-local campaigns explored distinct `blue-sdk-go` surfaces with grounded hypotheses while admitting nothing — the one live candidate was mechanically REFUSED (`MISSING_REPRODUCTION`). Live-campaign forensics found and fixed a real runtime waste defect: `detectRepeatedAction` inspected only a CONSECUTIVE streak, so one reproduction fingerprint was executed three times; `detectExhaustedAction` now blocks any fingerprint that already failed in the session. Validated implementation checkpoint `3d624fbcc42da808ce1c7e9cbc6b780b82d90820`; `npm test` 4429 passed / 0 failed / 15 skipped; `gate:local` FULL PASS 11/11 receipt `receipt:sha256:0d892de64df3e499898a8289`; `gate:clean` PASS on a fresh Node 20 clone, inner receipt `receipt:sha256:c6737eb5be31ce52f13647c4`; external CI was not run and is not claimed green. NOT claimed: previously unknown Alphaus bug discovery, EXACT rediscovery, DEV/NEXT/production, organizational approval, or parent programme completion |
| `W9_OWNER_LOCAL_DETERMINISTIC_REPRODUCTION_YIELD_STATUS` | `COMPLETE_LOCAL_NOT_CI_VERIFIED` — bounded host-derived `GO_VENDORED_PACKAGE_TEST` execution in disposable `bwrap --unshare-net` state, explicit `CURRENT_SOURCE_REPEATED_TEST_FAILURE` proof/admission distinct from historical `PRE_FAIL_POST_PASS`, host-owned finite retry dispositions, exact v2 provider-transport/tool-payload accounting and conservative v1 resume migration. Final `opencode-go/omen-alpha` HOUR_1 campaign bound on wall time with 26 unique targets, 28 grounded hypotheses, 1 candidate, 0 admissions / 1 correct `MISSING_REPRODUCTION` refusal. Implementation/local/clean anchor `bb28480c6a6969a06744c75c4c947851d5bece7c`; local receipt `receipt:sha256:6fb76272f121ec1bed5b74bf`; fresh Node 20 clean receipt `clean-receipt:sha256:d914db277a583699a1ff68c3`; strict EXACT and previously-unknown-defect status remain unproven. |
| `W10_REPRODUCTION_SURFACE_COVERAGE_AUTONOMOUS_YIELD_STATUS` | `COMPLETE_LOCAL_NOT_CI_VERIFIED` — the M0 census proved W9's 7/7 `NOT_AVAILABLE` was structural: a repository-major source index truncated to 32 entries showed the reasoner one repository with zero executable coverage, while 1,120 of 4,109 approved files (152 distinct targets) were already executable under the unchanged W9 executor. W10 added a bounded `nightwatch.reproduction-surface-map.v1` capability projection visible BEFORE a reproduction attempt, repository/executable-target diverse index selection, capability carried across investigations and restored on resume, sanitized `nightwatch.current-failure-evidence.v1` triage on qualifying repeated assertion failures, mechanical `nightwatch.w10-campaign-yield.v1` live metrics, and a host-owned `--repository` campaign scope that fails closed on an unapproved id and on a widened resume. Both candidate executor expansions (`GO_LOCAL_CACHE_PACKAGE_TEST`, one non-Go class) are evidence-based NO-GO: they unlock zero approved targets. Live `opencode-go/omen-alpha` HOUR_1 campaigns: reproduction attempts that reached real contained execution went from 0/7 (W9) to 6/6 census-scoped, 5/5, 5/5 and 4/6 broad, with `NOT_AVAILABLE` waste 7/7 → 0/6. Zero qualifying current-source failures, zero admissions, zero leakage, zero sibling writes; no previously unknown Alphaus defect is claimed |
| `W11_AUTONOMOUS_YIELD_PROOF_STATUS` | `PARTIAL_BLOCKED_EXTERNAL_PROVIDER` — the historical arm is COMPLETE and the unknown-yield arm is frozen but UNEXECUTED. Under frozen definition `sha256:824deef9922975feab5af69f` with `opencode-go/glm-5.3` (the W9/W10 provider `opencode-go/omen-alpha` no longer exists; the replacement was chosen by a preference list recorded BEFORE probing, first pass wins, later entries never probed), 14/14 leak-isolated historical cases evaluated in 66.8 min over 78 reasoner calls: **strict `EXACT_REDISCOVERY` 0, rate 0/13**, 10 near matches, 4 reproductions, 7 candidates, 3 mechanical admissions, **4 refused `MISSING_REPRODUCTION`**, 0 false positives, 0 `ENVIRONMENT_BLOCKED`, 0 leakage across 78 audited request blobs with the checker proven live by canaries. EXACT is 0 because `testMatch` failed in 13/13 while file recall reached 1.00 in 9 cases: the hidden failing test is absent from the entire visible context in all 8 fixtures and is a fix-CREATED file for mined cases (`git diff-tree` status `A`), so strict EXACT and zero leakage are in tension by construction. No threshold was tuned and no near match promoted. The unknown arm was frozen at `sha256:6145bd666dd08369ec38b018` before execution; its only run is invalid on provider grounds — 6/6 calls `REASONER_TIMEOUT` with zero provider response bytes — because the `opencode-go` subscribed namespace degraded mid-wave. The frozen provider was not swapped, the threshold not lowered and configuration not changed to force availability. Reproduction capability remains confined to `mobingilabs/ouchan` (1,120 executable files / 152 targets of 4,124 eligible), so investigation breadth is 8 repositories and execution breadth is 1. Sibling writes 0, DEV/NEXT/production contacts 0, no previously unknown Alphaus defect claimed |
| `W12_CURRENT_SOURCE_UNKNOWN_YIELD_STATUS` | `PARTIAL_BLOCKED_EXTERNAL_PROVIDER` — successor wave activated from `4e763f3f079863a262906a8b134539e309c8d054`; W11 remains frozen predecessor evidence. W12 froze `opencode-go/glm-5.3`, rebaselined all eight repositories at exact CURRENT SHAs, and froze the broad-plus-eight-scoped matrix at `sha256:8ea18e4fe5e9315271dd55df` before investigation. The broad run was valid and reached all eight repositories: 2 candidates, 2 reproduction attempts, 0 qualifying reproductions, 2 `MISSING_REPRODUCTION` refusals, 0 admissions. All eight scoped runs were provider-blocked before source actions (23 runtime timeouts and 33 nonzero exits across 56 provider failures); provider-invalid runs are excluded from zero-yield denominators. Leakage and sibling identity drift are 0; no Alphaus organizational novelty is claimed. Evidence: `.agent/tasks/nightwatch-current-source-unknown-yield-w12-v1/evidence/global-yield-aggregation.json` |
| `W13_PROVIDER_RESILIENT_CURRENT_YIELD_STATUS` | `COMPLETE — CURRENT-SOURCE YIELD MEASURED, MECHANICAL ADMISSIONS: 1` — successor wave from `34517c9ba11c97407168fe5879ee03794dfff3e3`; W11/W12 remain frozen predecessors. Phase A closed every locally closable W12 residual (budget mismatch D-139, `gate:local` timeout classification, ten-member provider taxonomy, census floor semantics, aggregate completeness, per-provider attribution, candidate/admission and failed-provider guards). Phases B executed the frozen 9-run matrix under a predeclared deterministic failover policy: generation 1 `opencode-go/glm-5.3` with two transitions, generation 2 (owner directive D-140) `opencode-go/muse-spark-1.3-contributor` at XHIGH effort, with D-141 completing runs 02/03/04 under the same freeze. Final aggregate: 9/9 valid runs, 67/67 investigations, 392 calls, 227 tool actions (floor), 15 contained reproduction attempts, 1 qualifying reproduction, 6 candidates, 5 refusals, **1 mechanical admission** (`mobingilabs/ouchan` candidate `c1`, `VERIFIED_REPRODUCTION`, reproductionCount 1; novelty `NOVELTY_AMBIGUOUS` via harness defect W13-DEF-01), 0 leakage, all eight sibling SHAs unchanged. Evidence: `.agent/tasks/nightwatch-provider-resilient-current-yield-w13-v1/evidence/global-yield-aggregation.json` |
| `TEST_INFRASTRUCTURE_PERFORMANCE_V1_STATUS` | `COMPLETE — DEVELOPMENT LOOP MATERIALLY ACCELERATED` — owner-authorized test/gate performance campaign activated from `8dd8b163b567b939b977649d6ba7c371cf230ee6`; W13 remains a frozen read-only predecessor. The campaign measures every named validation lane with host-load receipts, classifies and removes or reuses duplicated expensive work, adds per-file execution classes plus a shard runner with a mechanical coverage-equality proof, adds `gate:dev` and `gate:milestone` lanes that are explicitly not certification, optimizes the synthetic campaign (1,897/1,897 preserved) and the hardening probe harness (83 rules / 94 probes preserved), and closes with before/after timing evidence. Measured: synthetic 464.4 s serial -> 141.4 s at four weighted shards (1,897/1,897 preserved); canonical `npm test` a green coverage-proven shard run at 633.7 s; `gate:local` all twelve groups PASS at 635.7 s; `reviewStoreHardening` 524 s -> 55 s; remaining limit recorded: the serial semantic-compatibility group (364.2 s of the gate). Evidence under `.agent/tasks/nightwatch-test-infrastructure-performance-v1/evidence/` |
| `SUCCESSOR_CAMPAIGN_ENGINE_V1_STATUS` | `COMPLETE_LOCAL_NOT_CI_VERIFIED` — fresh evidence-led successor loop implemented shard execution-receipt integrity, child-process census indirection, run-evidence transaction integrity, proxy-event firewalling, DEV credential-use binding, per-shard temp isolation, and live-source test hermeticity. Final `gate:dev` / `gate:milestone` selected 5472 tests with 0 failures; local receipt `receipt:sha256:a55cac531dfae3c5f15ac598`; clean Node 20 receipt `clean-receipt:sha256:fe0bea35bf55cc3a885d98b6`. Popup L0 remains an honestly blocked lower-level target-admission design residual. |
...
LAST_SUBSTANTIVE_IMPLEMENTATION_SHA: b0f9b1f2ed9ccd2c47d7c4002ffb40804d5ec415
LAST_LOCALLY_VALIDATED_SHA: b08926b162a5d68aff6900f14ad636cd3a40ba86
LAST_CLEAN_VALIDATED_SHA: b08926b162a5d68aff6900f14ad636cd3a40ba86
...
LIVE_TASK_ID: nightwatch-successor-campaign-engine-v1
LIVE_PHASE: SUCCESSOR_CAMPAIGN_ENGINE_V1
...
### Control Center style and absence truth (current)

Two verification limits from the render-truth campaign and one real divergence
closed together. The source stylesheet guard excluded seven interpolation
prefixes and asserted concrete values for only three families, so the
`status-*` and `stage-*` families it renders were unasserted; those are now
asserted with `status-neutral` and `stage-neutral` stated as intentionally
base-only against the base pill and chip. The system map built
`node-${evidenceStatus}` and `edge-${evidenceStatus}` classes from the
13-value core evidence vocabulary while the stylesheet matched only four
historical values, so every map tone rule was dead; the interpolation and the
dead rules were removed (no pixel change) and `.map-node`, `.map-edge` and
`.node-selected` remain.

A new runtime computed-effect sweep in the browser lane toggles every class
the built composition renders off its carrying element and requires a
computed property to change, with a reasoned base-only list asserted in both
directions. It found and fixed three real cascade defects:
`.run-detail-panel` lost its intended border to the later `.panel` rule (now
`.panel.run-detail-panel`), and `.campaign-metrics` and `.graph-node-neutral`
were removed as declarations that restated their base classes. Native form
controls are excluded with a reason: this browser environment forces their
computed colours, so button classes are covered statically. The page's
`style-src 'self'` CSP blocks injected styles, so transition suppression uses
a CSSOM rule on the app's own sheet.

The render harness gained an absence pass: the generator records every array
at every depth, and emptying each one must change the DOM of a view that can
receive it. Every recorded collection was absence-observable; the exemption
list is empty, vacuity-guarded and mutation-proven.

Certification: implementation `88e3c3fb52937ff303b0944cc22cfee624bf807e`;
`gate:local` all eleven groups PASS with receipt
`receipt:sha256:7945d6ad715fab8479e36d2a`; UI typecheck, 63 tests and build
