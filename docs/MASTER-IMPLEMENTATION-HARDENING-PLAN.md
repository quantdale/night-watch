...
- **Priority / category / confidence / status:** P1 external prerequisite and release-path truth; CONFIRMED by measurement (`POSITIVE_DEPLOYMENT_FACTS: 0`); repository work **IMPLEMENTED LOCAL/SYNTHETIC** under `nightwatch-production-completion-programme-v1` G10; **external prerequisite UNMET**, owner decisions pending.
...
- **Pending owner decisions (unclaimed).** 10.6 — obtain or refuse the read-only access and refuse any manifest from another route. 10.13 — if access will not be granted, record C-13 and C-14 terminal and close the production track honestly. Until 10.6 resolves, the track reports `EXTERNAL_PREREQUISITE_UNMET`, not `AWAITING_AUTHORIZATION`.
