...
### NW-16 — Production track prerequisites and the C-08b critical path

- **Priority / category / confidence / status:** P1 external prerequisite and release-path truth; CONFIRMED by measurement (`POSITIVE_DEPLOYMENT_FACTS: 0`); repository work **IMPLEMENTED LOCAL/SYNTHETIC** under `nightwatch-production-completion-programme-v1` G10; **external prerequisite UNMET**; owner decisions taken 2026-09-12 (10.6/10.13).
- **Affected surfaces:** `src/core/source/deploymentManifestReader.ts`, `src/core/prodObserve/deploymentFactAdmission.ts`, `src/core/productionTrack/**`, `src/core/prodObserveP1/session.ts` and `safeReceipt.ts`, their focused suites, `docs/ROADMAP.md`, `docs/C12-OPERATOR-RUNBOOK.md` (referenced), `openspec/changes/nightwatch-production-completion-programme-v1/tasks.md`.
- **Evidence:** the census ledger measures `POSITIVE_DEPLOYMENT_FACTS: 0` with `DEPLOYMENT_BINDINGS: 1851`. The mochi manifests are not locally available (verified four independent ways), so U-1/U-2 remain `C08B_BLOCKED_BY_ORGANIZATIONAL_ACCESS`. The guard was mutation-measured: removing `PRODUCTION_READ_NO_DEPLOYMENT_FACT` failed five guard-refusal cases; restored, all fourteen pass. The manifest reader refuses every non-approved source/path and every ambiguous, templated or host-less mapping, and binds facts to `mochi @ SHA : path` with an `ev:sha256` digest.
- **Problem, impact, root cause:** C-13 and C-14 were described as `NOT_AUTHORIZED`, which reads as "awaiting a decision". The measured truth is stronger: with zero positive facts no authorization can unblock them, so the production track's critical path is an organizational access request, not more code. The status vocabulary and the construction-time guard now say so structurally.
- **Recommended implementation (done locally):** refuse production reads at construction without a positive `DEPLOYMENT_FACT`; report the capability unavailable with the count as the reason; record per-stage repository work and external prerequisites as data with `EXTERNAL_PREREQUISITE_UNMET` distinct from `AWAITING_AUTHORIZATION`; keep D-4 unloadable with a per-stage/per-session future grant shape; abort passive sessions on Nightwatch-attributable traffic and emit safe `INTERNAL_ERROR` receipts. Bounded read-only access to `mochi` at `services/{env}/{appproxy,serviceproxy}/ingress.yaml` remains the one external ask; no write, no other path, no credential beyond read.
- **Constraints and compatibility:** no production contact, no network, no credentials, no sibling writes; no capability added here makes production loadable; `config/environments/production.json` and `SUPPORTED_ENVIRONMENTS` are untouched; the derivation is exercised only against synthetic fixtures. The C-11 chain's denial vocabulary is not extended by the construction-time guard, which runs before the chain.
- **Tests and validation:** `tests/unit/c13ProductionReadGuard.test.ts`, `tests/unit/c08bDeploymentManifestReader.test.ts`, `tests/unit/productionTrackReadiness.test.ts`, and the extended `tests/unit/p1Attribution.test.ts`; root `typecheck` and `hardening:check`.
- **Acceptance:** an authorized inferred route refuses with `PRODUCTION_READ_NO_DEPLOYMENT_FACT` and constructs nothing; the capability's reason is the measured count; an ambiguous manifest yields an explicit unknown and increments no fact; a changed manifest refuses rebinding; the track reports the correct blocker kind per stage; production remains unloadable by default; a Nightwatch-attributable request aborts and invalidates session evidence.

- **External prerequisite and lead time (C-08b).** Read-only `mochi` access at the exact bounded manifest paths is the critical-path prerequisite for C-13 and C-14, the only items whose absence is structural rather than decisional. Lead time is organizationally bounded and not measurable from repository evidence; the block has persisted from C-08 (2026-08) through C-11 and MA-8/F-13 (2026-09). The request is filed so the organizational clock starts; planning assumption is at least one review cycle (≥ 4 weeks), revisited at the next owner sync.
- **Owner decisions taken (10.6/10.13, 2026-09-12).** The authorized read-only `mochi` access is UNAVAILABLE: no `mochi` checkout and no `services/{env}/{appproxy,serviceproxy}/ingress.yaml` exists under the sibling root, so no manifest was read and no deployment fact is claimed. Per the 10.13 path, C-13 and C-14 are recorded terminal on the programme roadmap and the release advance-conditions surface, with that reason (D-134); the track's own status vocabulary is unchanged and continues to report `EXTERNAL_PREREQUISITE_UNMET` rather than `AWAITING_AUTHORIZATION`.
